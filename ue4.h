...
namespace UE4
{
    ...
    #pragma warning( pop )
        ...
        inline DWORD FindOffsetOld(const char* VariableFullName)
        {
            UE4::UObject* VariableObj = this->FindObjectByFullName(VariableFullName);
            if (VariableObj)
            {
                __int64 VariableOffsetAddr = __int64(VariableObj) + 0x44;
                uint32_t VarOffset = *(uint32_t*)(VariableOffsetAddr);
                return VarOffset;
            }
            return NULL;
        }
